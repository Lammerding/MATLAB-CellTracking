...
if finishedCells(i).Rupture(timePoints) > 0
    unfinishedRuptures(end + 1).CellNumber = i;
    unfinishedRuptures(end).Start = find(finishedCells(i).Rupture <= 0, 1, 'last') + 1;
    unfinishedRuptures(end).Position = s;
    unfinishedRuptures(end).Size = c;
end
